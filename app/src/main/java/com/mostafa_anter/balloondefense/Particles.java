...
public class Particles {
	...
	Bitmap particle_img;
	int number = 100;//number of particles ------CHANGE ME for denser clouds
	...
	public Particles(Bitmap particle_img, int screenwidth, int delay, int speed, int particle_direction, int initialx_offset, int initialy_offset, int particle_life) {
		...
		for (int i = 0; i < number; i++) {
			...
			alpha[i] = (int) (Math.random() * 100);//alpha of each particle-------CHANGE ME to change transparency
		}
	}
